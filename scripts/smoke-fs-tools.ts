...
async function call(name: string, args: unknown): Promise<{ ok: boolean; content: string }> {
  const tool = getTool(name);
  if (!tool) throw new Error(`tool not registered: ${name}`);
  const raw = await tool.run(args as never);
...
async function main(): Promise<void> {
  ...
  try {
    ...
    // ── file_read ──────────────────────────────────────────────────────────
    const r1 = await call("file_read", { path: f });
    ...
    _resetFileHistoryForTesting();
    const eBlind = await call("file_edit", { path: f, oldString: "row", newString: "X" });
    must(
    ...
    const g1 = await call("glob", { pattern: "*.ts", cwd: dir });
    ...
    const gm1 = await call("grep", { pattern: "export const", path: dir });
    must(
    ...
    const gm2 = await call("grep", { pattern: "const A", path: dir, output_mode: "content" });
    must(gm2.ok && /a\.ts.*:1:.*const A/.test(gm2.content), "grep content emits path:line:body");

    // ── grep: count mode ───────────────────────────────────────────────────
    const gm3 = await call("grep", { pattern: "export", path: dir, output_mode: "count" });
    ...
    const gm4 = await call("grep", { pattern: "definitelyNotInAnyFile", path: dir });
    ...
    const wBig = await call("file_write", { path: join(dir, "big.bin"), content: big });
