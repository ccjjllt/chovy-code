...
- **`memoryStore.upsertFromCheckpointFile` 真正写入**：step-24 已上线 `MemoryStore.upsert`，但 checkpoint → MemoryRecord 的解析在 step-27（SCW）一并接入更合理；coordinator 留 `// TODO step-24/25` 注释占位；
...
### 6.2 step-24/25 MemoryStore 接入

coordinator 写盘后留 `// TODO step-24/25: memoryStore.upsertFromCheckpointFile(latest)` 占位。step-25 落地 `parser.ts` 已有 `parseMemoryDocument`，可直接解析 `latest.md` 的 frontmatter+bullets 段，调 `MemoryStore.upsertMany(records)`。本步不做，避免与 SCW 的注入路径耦合。
