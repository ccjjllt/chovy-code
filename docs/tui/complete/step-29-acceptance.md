...
| `src/engine/queryEngine.ts` | step-0 `runSkillRound` 调用 + `goalObjective?` + `session?` 字段（**585 行 / 600 硬限，2026-06-19 复验 smoke 口径**）|
...
| 10 | `queryEngine.ts ≤ 600 行` 硬限 | ✅ smoke §13：585 行 |
...
- `queryEngine.ts ≤ 600 行` 硬限继续守恒（复验 585）
