...
- 解码：Bun 内置 `Bun.file().arrayBuffer()` 读 GIF → 用 `omggif` 或自实现 LZW 解码 → ARGB 帧序列。
...
export interface PaletteCommand {
  ...
  category: "推荐"|"会话"|"模型"|"主题"|"设置"|"工具";   // 中文 + 英文映射 via i18n
  hotkey?: string;            // "Ctrl+X L" — 与 keybindings 单源同步
...
1. **不复制 mimo 的橙色高亮**——chovy-code 用主题 `accent`（蓝）做高亮背景。
...
{
  name: "ChovyDefault",
  primary:   "#8B5CF6",   // violet-500（吉祥物边框 / 高亮文字）
  accent:    "#3B82F6",   // blue-500（命令面板高亮 / 焦点边框）
  bg:        "default",   // 跟随终端
  fg:        "#E5E7EB",   // 浅灰
  muted:     "#6B7280",   // 灰
  success:   "#10B981",
...
找最近的 ANSI 16 色名（`magenta`/`cyan`/...）作为 fallback，**真彩色终端**走 `<Text color="#8B5CF6">`（Ink 5+ 支持）。
...
`Locale = "zh-CN" | "en-US"`，**默认 zh-CN**（图 4 风格）。所有 **UI 文本** 调用 `t(key, params?)`。
...
- `/lang zh` / `/lang en` slash 命令（命令名英文，参数枚举也英文）；
- `Ctrl+L` 全局快捷键；
- Settings → Language（标题"语言"中文，但选项 value 仍是 `zh-CN` / `en-US` 英文 ID）；
- 启动时 `process.env.LANG` / `process.env.LC_ALL` 探测兜底（一次性，不实时跟随）。

### 与 mimo 差异

- 不抄 mimo 的 i18next 框架（重）——chovy-code 自实现 ~80 行 `t()`：dictionary lookup + `{param}` 占位符替换 + missing-key fallback。
- 不内置 RTL（阿语 / 希伯来语），明确写在 KNOWN-LIMITATIONS。
- **新加 key 必须同时加 zh-CN + en-US**——CI smoke 校验 key 集合相等，缺一即 fail（step-32 要点）。
...
| 4 | mimo 命令面板的橙色 `#FF7A00` 高亮 | mimo dialog-settings | `theme.accent`（默认蓝） |
| 5 | mimo 的 `i18next` 依赖 | mimo language context | 自实现 ~80 行 `t()` |
...
## 7. 创新与既有 5 项创新（ATP/SwarmR/TMT/SCW/CSG）的关系
